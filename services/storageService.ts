...
const syncToGoogleSheet = async (recordType: 'EMPLOYEE' | 'CATEGORY', operation: 'SAVE' | 'DELETE', data: any, companyId?: string) => {
    try {
        ...
        // CATEGORIES always fallback here because they don't have a specific companyId context usually
        if (!targetUrl && settings.appScriptUrl) {
            targetUrl = settings.appScriptUrl;
        }

        if (!targetUrl) {
            console.warn(`SYNC SKIPPED: No App Script URL found. Please configure 'Global Settings' or 'Company Settings'.`);
            return;
        }
...
export const StorageService = {
  ...
  saveCategory: (category: Category): ApiResponse<Category> => {
    try {
      ...
      // Ensure sync is called for Category updates
